...
s = eta / (1-eta)
print s
ap = -zs.hbar * atom.k * atom.G / (2 * atom.m) / A
zx = 0.58 / L
tp = (vfp - v0p)/ (ap * eta)
zz = v0p*tp + 0.5 * eta * ap * tp**2 
print v0p, vfp, ap
print tp, zx, zz*L, (vfp**2 - v0p**2)/(2 * eta * ap)*L

zl = np.linspace(0, zz, 101)
# bx = np.sqrt(v0p**2 + 2*ap*eta*zl)
bx = np.sqrt(v0p**2 + 2*ap*eta*zl)
# bx = v0p + ap / v0p * zl
# bx = (bx - v0p) / 2 + v0p
...
fields = lambda z : np.sqrt(v0p**2 + 2*ap*eta*z)

y0 = [0, v0p]
tp *= 2
t = np.linspace(0, tp, 201)
s = 1
args = (ap, s, detu, fields)
y = fly((y0, t, args))
...
# pl.subplot(211)
# pl.plot(z/L, vfield/V)
pl.plot(zl, bx, '.-')
# pl.plot(z/L, v/V)
# pl.plot(z/L, res/V, 'k-', linewidth=2)
pl.plot(y[:, 0], y[:, 1], 'r-')
pl.plot(y2[:, 0], y2[:, 1], 'b-')
pl.xlim([0, 0.6/L])

# pl.subplot(212)
# pl.plot(y[:, 0], fields(y[: ,0]) - y[:, 1], 'r.-')
# pl.xlim([0, 0.6/L])
